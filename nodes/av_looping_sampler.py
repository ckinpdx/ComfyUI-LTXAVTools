...
def _apply_adain(video_out, adain_ref, factor, per_frame=False):
    """
    Normalize video_out channel statistics to match adain_ref, then lerp.
    video_out / adain_ref: [B, C, T, H, W]
    factor=0.0 → no change; factor=1.0 → full normalization.
    per_frame=False → normalize per-channel globally (uses first chunk as ref).
    per_frame=True  → normalize per-frame per-channel (uses normalizing_latents).
    """
    if factor == 0.0:
        return video_out
    t = video_out.clone()
    for b in range(t.shape[0]):
        for c in range(t.shape[1]):
            if not per_frame:
                r_sd, r_mean = torch.std_mean(adain_ref[b, c], dim=None)
                i_sd, i_mean = torch.std_mean(t[b, c], dim=None)
                if i_sd > 1e-8:
                    t[b, c] = ((t[b, c] - i_mean) / i_sd) * r_sd + r_mean
            else:
                for f in range(t.shape[2]):
                    rf = min(f, adain_ref.shape[2] - 1)
                    r_sd, r_mean = torch.std_mean(adain_ref[b, c, rf], dim=None)
                    i_sd, i_mean = torch.std_mean(t[b, c, f], dim=None)
                    if i_sd > 1e-8:
                        t[b, c, f] = ((t[b, c, f] - i_mean) / i_sd) * r_sd + r_mean
    return torch.lerp(video_out, t, factor)
...
class LTXVAVLoopingSampler:
    ...
    def _sample_first_chunk(
        ...
        if adain_ref is not None and adain_factor > 0.0:
            video_out = _apply_adain(video_out, adain_ref, adain_factor, adain_per_frame)
        ...
        for i_t, (v_start, v_end) in enumerate(zip(starts, ends)):
            ...
            if adain_factor > 0.0:
                if normalizing_latents is not None:
                    ...
                    chunk_adain_ref = adain_ref
            else:
                chunk_adain_ref = None
            ...
            if v_start == 0:
                ...
                # Store first chunk output as fallback reference
    ...
    def sample(
        ...
        for v in range(vertical_tiles):
            for h in range(horizontal_tiles):
                ...
                tile_norm = None
                if optional_normalizing_latents is not None:
        ...
        # Trim to the input latent's frame count — the last chunk's window can
